...
public class InvoiceRepository : IInvoiceRepository
{
    ...
    public InvoiceRepository(LuminaDbContext db) => _db = db;
    ...
    {
        // Same race-condition caveat as SaleRepository.NextTicketNumberAsync
        // (Phase 2/3 notes) — counts existing rows, not safe under concurrent
        // invoice issuance at the same store. Not fixed here either.
        var count = await _db.InvoiceRecords.CountAsync(i => i.StoreId == storeId, ct);
        return $"FA-{count + 1:D6}";
    }
