...
### 1.1 Starting a cart — `IPosSaleService.CreateCartAsync` (NEW)

Real gap closed: until now there was no contract method for creating a `Cart` at
all — `AddLineAsync`/etc. all assumed a `cartId` already existed, with nothing
defining where that id came from. **This is the actual entry point for a real
POS sale flow** — UI calls this first, gets back a `CartId` (see 1.2), and passes
that into every subsequent call for this sale.

```csharp
var cart = await posSaleService.CreateCartAsync(
    new CreateCartRequest(StoreId: session.StoreId, CustomerId: null));
// cart.CartId is now the id to use for AddLineAsync/RemoveLineAsync/CompleteSaleAsync
```

`StoreId` comes from `IUserSession.StoreId` (Contracts.Auth) — the UI already has
this from the logged-in session, no separate store lookup needed. `CustomerId` is
optional; a POS sale can be anonymous.

### 1.2 `ICartSummary.CartId` (NEW)

`ICartSummary` gained a `CartId` field — additive, existing bindings against this
interface still compile. Every `ICartSummary` returned by `CreateCartAsync`,
`AddLineAsync`, and `RemoveLineAsync` carries the same `CartId` for a given cart;
UI should treat it as the stable identity of the cart across the whole flow.
