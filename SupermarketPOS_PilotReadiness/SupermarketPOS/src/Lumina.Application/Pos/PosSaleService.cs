...
public sealed class PosSaleService : IPosSaleService
{
    ...
    public PosSaleService(
        ...
        StockLedgerService stockLedger, CashDrawerService cashDrawer, IUnitOfWork unitOfWork)
    {
        _carts = carts;
    ...
    public async Task<ICartSummary> CreateCartAsync(CreateCartRequest request, CancellationToken ct = default)
    {
        var cart = new Cart(Guid.NewGuid(), request.StoreId);
        if (request.CustomerId.HasValue)
            cart.SetCustomer(request.CustomerId);
        ...
        Guid cartId, CompleteSaleRequest request, CancellationToken ct = default)
    {
        ...
        // FIXED as part of the correctness-hardening pass: the stock-check
        // through the final SaveChangesAsync now runs inside a real database
        // transaction (IUnitOfWork, SQLite BEGIN IMMEDIATE) — this closes the
        // stock-availability race flagged since Phase 4, where two concurrent
        // sales of the last unit of a product could both pass the
        // availability check before either decremented. See
        // GAPFIXES_NOTES.md for the empirical verification of why this works.
        //
        // SaleRejectedInternalException is control flow ONLY within this
        // transactional block — it lets every rejection path inside the
        // transaction (stock, tender validation, mixed-tender, fiscal chain
        // integrity) roll back through one single catch, rather than each
        // early-return site needing to remember to call RollbackAsync itself.
        ...
        try
        {
            foreach (var group in cart.Lines.GroupBy(l => l.ProductId))
            {
                ...
                if (!await _stock.IsAvailableAsync(cart.StoreId, group.Key, totalQty, ct))
                    throw new SaleRejectedInternalException(
                        RejectionCode.StockUnavailable, $"Insufficient stock for product {group.Key}.");
            }

            var tenderTotal = request.Tenders.Sum(t => t.Amount);
            if (tenderTotal != cart.Total)
            {
                throw new SaleRejectedInternalException(RejectionCode.PaymentValidationFailed,
                    $"Tender total ({tenderTotal}) does not match sale total ({cart.Total}).");
            }

            // TenderType.MixedCashCard on a SINGLE TenderLine is rejected outright —
            // Phase 6's cash-drawer reconciliation needs to know exactly how much of
            // a sale's tenders was physical cash, and a "mixed" line gives no way to
            // split that. A genuinely mixed payment must be sent as two separate
            // TenderLines (one Cash, one Card) — the request/response shape already
            // supports that (Tenders is a list). Rejecting here rather than guessing
            // a 50/50 split or silently treating the whole amount as cash/non-cash,
            // either of which would corrupt register-close reconciliation.
            if (request.Tenders.Any(t => t.Type == TenderType.MixedCashCard))
            {
                throw new SaleRejectedInternalException(RejectionCode.PaymentValidationFailed,
                    "MixedCashCard is not a valid single tender line — split into separate Cash and Card TenderLines.");
            }
            ...
            // using Store.TimeZoneId before being sent to the fiscal layer.
            ...
            // TipoFactura: defaulted to "F2" (factura simplificada / ticket) —
            // NOT independently confirmed against AEAT's invoice-type code
            // list (L2). See docs/PHASE2_3_NOTES.md.
            const string invoiceType = "F2";

            FiscalRecordResult fiscalResult;
            try
            {
                fiscalResult = await _fiscal.GenerateAsync(
                    sifBoundaryId,
            ...
            {
                // Maps to RejectionCode.FiscalChainError — an enum value that
                // has existed since Phase 3 with no code path that ever
                // triggered it until the chain-integrity check was implemented.
                throw new SaleRejectedInternalException(RejectionCode.FiscalChainError, ex.Message);
            }

            sale.AttachVeriFactuRecord(fiscalResult.VeriFactuRecordId);

            // Stock movements, cash movement, and the Sale itself all commit
            // together via ONE final SaveChangesAsync + the outer transaction's
            // CommitAsync below — a crash anywhere in this section rolls the
            // whole thing back, never leaving a partial/inconsistent state.
            await _stockLedger.RecordSaleMovementsAsync(
                store.Id, sale.Id, cart.Lines.Select(l => (l.ProductId, l.Quantity)), ct);

            var cashTenderAmount = request.Tenders.Where(t => t.Type == TenderType.Cash).Sum(t => t.Amount);
            await _cashDrawer.RecordSaleCashTenderAsync(register.Id, sale.Id, cashTenderAmount, ct);
        ...
        catch (SaleRejectedInternalException ex)
        {
            await _unitOfWork.RollbackAsync(ct);
            return Rejected(ex.RejectionCode, ex.Reason);
        }
        catch
        {
            await _unitOfWork.RollbackAsync(ct);
    ...
    /// <summary>Internal control-flow-only exception — never crosses the
    /// boundary of CompleteSaleAsync. Exists so every rejection path inside
    /// the transactional block above can roll back through one catch clause
    /// instead of each site remembering to call RollbackAsync itself.</summary>
    private sealed class SaleRejectedInternalException : Exception
    {
        public RejectionCode RejectionCode { get; }
        public string Reason { get; }
        public SaleRejectedInternalException(RejectionCode rejectionCode, string reason)
        {
