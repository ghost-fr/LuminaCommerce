...
if (tenant.Nif == PilotSafetyGuard.KnownDemoNif)
{
    Console.WriteLine();
    Console.WriteLine("*** THIS IS DEMO/SEED DATA — do not point a Pilot or Production build");
    Console.WriteLine("*** (LUMINA_BUILD_MODE=Pilot/Production) at this database. Those builds");
    Console.WriteLine("*** will refuse to start against this NIF (PilotSafetyGuard) — by design.");
    Console.WriteLine("*** Use --nif to seed a different value if you need one for a real rehearsal.");
}
...
{
    // References PilotSafetyGuard.KnownDemoNif rather than a separately
    // hardcoded literal — single source of truth. If this default ever
    // changes, PilotSafetyGuard's check changes with it automatically;
    // they can never silently drift apart.
    public string Nif { get; set; } = PilotSafetyGuard.KnownDemoNif;
