...
against `ICurrentTenantProvider`, same pattern as `AuthService`. `PosSaleService` calls
the public `FindByBarcodeAsync` directly (the earlier internal-method workaround was
removed). No known gaps remain in this contract as of this version.
