...
public sealed class QuoteService : IQuoteService
{
        ...
        Guid quoteId, Guid idempotencyKey, CancellationToken ct = default)
    {
        ...
        {
            // Same chain-integrity pre-check as PosSaleService. Nothing was
            // persisted (Quote hasn't been marked converted, Invoice hasn't
            // been added). ConvertToInvoiceResult has no dedicated fiscal-
            // error status code (unlike CompleteSaleResult's RejectionCode),
            // so this surfaces as an ordinary Rejected with the exception's
            // message — a future contract revision could add a matching
            // status enum if the UI needs to distinguish this case visually.
            return new ConvertToInvoiceResult(ConvertToInvoiceStatus.Rejected, null, null, null, ex.Message);
        }
