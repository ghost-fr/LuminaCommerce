...
public sealed class PosSaleService : IPosSaleService
{
    ...
    public PosSaleService(
        ...
        StockLedgerService stockLedger, CashDrawerService cashDrawer)
    {
        _carts = carts;
        ...
        Guid cartId, CompleteSaleRequest request, CancellationToken ct = default)
    {
        ...
        // Same atomicity discipline: cash movement recorded before the final
        // SaveChangesAsync, so it commits together with the Sale — a crash
        // between the two can't leave a fiscally-recorded sale with no matching
        // cash-drawer entry (or a phantom cash entry with no sale behind it).
